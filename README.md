...
> **Phase 3는 의도적으로 범위에서 제외했습니다.** 2차 시장의 가격-시간 우선 매칭은 별도 프로젝트 MINI-Exchange에서 이미 구현했기에, 본 프로젝트는 중복을 피해 **"규제 컴플라이언스를 코드로 강제"**라는 차별점에 집중했습니다. 온체인 매칭은 가스 비용이 크므로, 확장 시에는 오프체인 매칭 + 온체인 정산 구조가 현실적입니다.
