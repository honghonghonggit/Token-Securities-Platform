...
> **Phase 3는 의도적으로 범위에서 제외했습니다.** 2차 시장의 가격-시간 우선 매칭은 별도 프로젝트 [MINI-Exchange](https://github.com/honghonghonggit/MINI-Exchange)에서 이미 구현했기에, 본 프로젝트는 중복을 피해 **"규제 컴플라이언스를 코드로 강제"**라는 차별점에 집중했습니다. 온체인 매칭은 가스 비용이 크므로, 확장 시에는 오프체인 매칭 + 온체인 정산 구조가 현실적입니다.
...
- **범위 설정**: Phase 3(2차 시장 매칭)는 의도적으로 범위에서 제외했다. 가격-시간 우선 매칭은 이미 [MINI-Exchange](https://github.com/honghonghonggit/MINI-Exchange)에서 구현했기에 중복을 피하고 "규제 컴플라이언스를 코드로 강제"라는 차별점에 집중했다. 다만 확장한다면, 온체인 매칭의 가스 비용을 고려해 오프체인 매칭 + 온체인 정산 구조가 현실적이라고 판단한다.
